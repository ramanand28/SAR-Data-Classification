...
def trainNN():
    ...
    for image in input_images:
        ...
        y = imageio.imread('/optical/{}'.format(image))
        #print(y)
        y = label_creator(y)
        #print(y)
        ...
        #print(min(y))
        #print(X[0].shape)
        model.fit(X, y, batch_size = 4096,epochs=5)
...
def reports (X_test,y_test):
    ...
    target_names = ['land', 'water bodies', 'coconut trees']
...
for i in range(img.shape[0]):
    for j in range(img.shape[0]):
        if img[i][j] == 1:
            pred_img[i][j] = (255,0,0)
        elif img[i][j]==2:
            pred_img[i][j] = (0,0,255)
        #else:
         #   pred_img[i][j]=(0,0,0)
        elif img[i][j] == 3:
            pred_img[i][j] == (0,0,0)
res1 = Image.fromarray(pred_img, mode = 'RGB')
with open(os.path.join('/Users/jitendramouli/Desktop/mouli/', 'pred.jpg'), 'w') as f:
    res1.save(f)
...
def avg_accuracy():
    ...
    for image in input_images:
        #print(image.type)
        X_test = imageio.imread('/home/sno/Desktop/sar_imagery/trial3/input/{}'.format(image))
        y_test = imageio.imread('/home/sno/Desktop/sar_imagery/trial3/input/{}'.format(image))
        #print(y)
        ...
        os.chdir('/home/sno/Desktop/sar_imagery')
